...
Common.prepareAppConf = function(opts, app) {
  ...
  console.log(`Total key length: ${Object.keys(process.env).length}`);

  const filtered = filterEnv(process.env);
  console.log(`Filtered key length: ${Object.keys(filtered).length}`);

  function filterEnv(envObj){
    var new_env = {};
    var filterEnvs = app.filter_env != null ? app.filter_env : [];

    var allowedKeys = filterEnvs.concat([
      'TCP', 'HTTP', '_PORT', 'SERVICE_HOST'
    ]).reduce((acc, current) =>
 ...
 * @param {string} confString  contents of th e config file
